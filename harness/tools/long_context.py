"""
long_context_read — route large content through Llama 4 Scout's 10M-token context window.

Use this when content is too large for standard models (whole codebases, massive
document sets, long conversation logs, multi-file dumps). Scout ingests the full
...
_provider: Any = None
_model: str = "meta-llama/llama-4-scout"
...
@registry.register(
    ...
    description=(
        "Send a large body of text to Llama 4 Scout (10M-token context window) and get a "
        "targeted answer. Use this when content is too big for standard models: entire "
        "codebases, large document collections, massive log files, multi-file dumps. "
        "Scout reads the full content in one shot — no chunking needed. "
        ...
        "properties": {
            "content": {
                ...
                "description": (
                    ...
                    "or any text content. No size limit enforced — Scout handles it."
                ),
            },
            "question": {
                "type": "string",
    ...
    **kwargs,
    ...
    if _provider is None:
        return (
            ...
            "Set OPENROUTER_API_KEY or HF_TOKEN and restart."
        )
    ...
    try:
        ...
        return text or "(Scout returned an empty response)"
